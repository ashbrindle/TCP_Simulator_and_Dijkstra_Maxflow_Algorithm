...
class MaxFlow(Dijkstra): #inherits from Dijkstra class
    ...
    def return_near_neighbour(self):
        ...
        return Dijkstra.return_near_neighbour(self) # calls dijkstra to return the nearest neighbours list
    ...
    def remove_flow_capacity(self):
        ...
        for index, node in enumerate(self.route):   # enume rates through the shortest path in the network
            if index + 1 != len(self.route):    # checks if the position is at the end of the path
                ...
                if self.original_network[next_node_path][cur_node_path] == 0:
...
if __name__ == '__main__':
    m = MaxFlow()
    m.populate_network("network5.txt")   # populates the network with the provided "network.txt" file
